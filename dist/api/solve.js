...
export async function solveKinematics(token, joints, tasks) {
...
async function solveKinematicsWithStream(token, robotDat, tasks) {
    const steps = [];
    const body = {
        robot_dat: robotDat,
...
function handleSolverEvent(event) {
    switch (event.type) {
        case "start":
...
async function uploadJointData(token, joints) {
    const lines = [];
    // ジョイントの数
    ...
    const file = new File([content], 'joints.dat', { type: 'text/plain' });
